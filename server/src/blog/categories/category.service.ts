...
export class CategoryService {
    ...
    async createBlog(createCategoryDto: createCategoryDto): Promise<Categories> {
        ...
        const lowerName = title.toLocaleLowerCase();

        // Find the category if it already exist it won't allow to re-create
        const category = await this.categoryModel.findOne({ title: lowerName })
        if (category) {
            throw new ConflictException('Category Already Exist')
        }

        // Create the new category
        const createCategory = await this.categoryModel.create({ title: lowerName })
    ...
    async updateCategory(id: string, updateCategoryDto: updateCategoryDto): Promise<Categories> {
        try {
            ...
            const updatedCategory = await this.categoryModel.findByIdAndUpdate(id, { title: lowerName })
