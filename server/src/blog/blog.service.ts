...
export class BlogService {
    ...
    async findAllBlogs(): Promise<Blog[]> {
        return await this.blogModel.find().populate('category user')
            .populate({
                path: 'reaction',
                populate: {
                    path: 'user',
    ...
    async findOne(id: string): Promise<Blog> {
        ...
        const blog = await this.blogModel.findById(id).populate('category reaction comments').populate('user', '-password').populate({
            path: 'comments',
            populate: {
                path: 'user',
                model: 'User',
